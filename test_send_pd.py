...
def init_vidcap():
    ...
    while True:
        ...
        if success: 
            ...
            # print(mf_doom)
            # send_pd(str(mf_doom))
            if mf_doom > 0 and mf_doom < color_range*1: 
                send_pd(str(C))
            
            elif mf_doom > color_range*1 and mf_doom < color_range*2: 
                send_pd(str(F))
            
            elif mf_doom > color_range*2 and mf_doom < color_range*3: 
                send_pd(str(D))
            
            elif mf_doom > color_range*3 and mf_doom < color_range*4: 
                send_pd(str(G))
            
            elif mf_doom > color_range*4 and mf_doom < color_range*5: 
                send_pd(str(E))
            
            elif mf_doom > color_range*5 and mf_doom < color_range*6: 
                send_pd(str(A))
            
            elif mf_doom > color_range*6 and mf_doom < color_range*7: 
                send_pd(str(B))
